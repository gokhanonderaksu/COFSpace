...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/OnlyCoRECOF - HighHenry/CoRECOF - O2 - 5 BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('O2-5 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['O2-5 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.008006855096806122
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    XGBRegressor(learning_rate=0.1, max_depth=3, min_child_weight=1, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.45, verbosity=0)
)
...
X_unseen = pd.read_csv('C:/Users/Lenovo/Desktop/KodSystems/TPOT/Unseen/Unseen - O2 -  Henrys - 6872.csv', sep=',')
