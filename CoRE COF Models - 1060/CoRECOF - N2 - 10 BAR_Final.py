...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/OnlyCoRECOF/CoRECOF - N2 - 10 BAR.csv', sep=',', dtype=np.float64)
...
# Average CV score on the training set was: -0.025516257570466255
exported_pipeline = make_pipeline(
    MinMaxScaler(),
    SelectFromModel(estimator=ExtraTreesRegressor(max_features=0.8, n_estimators=100), threshold=0.0),
    StackingEstimator(estimator=XGBRegressor(learning_rate=0.01, max_depth=9, min_child_weight=4, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.05, verbosity=0)),
    StackingEstimator(estimator=RidgeCV()),
    RobustScaler(),
    ExtraTreesRegressor(bootstrap=False, max_features=0.6500000000000001, min_samples_leaf=2, min_samples_split=8, n_estimators=100)
)
...
X_unseen = pd.read_csv('C:/Users/Lenovo/Desktop/KodSystems/TPOT/Unseen/Unseen - N2 -  Henrys - 11467.csv', sep=',')
