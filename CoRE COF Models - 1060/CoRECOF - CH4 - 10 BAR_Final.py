...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/OnlyCoRECOF/CoRECOF - CH4 - 10 BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('CH4-10 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CH4-10 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.17788132807979287
exported_pipeline = make_pipeline(
    ZeroCount(),
    XGBRegressor(learning_rate=0.1, max_depth=3, min_child_weight=3, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.6000000000000001, verbosity=0)
)
