...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/OnlyCoRECOF - HighHenry/CoRECOF - H2 - 5 BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('H2-5 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['H2-5 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -2.4709514109689014e-05
exported_pipeline = make_pipeline(
    StandardScaler(),
    StackingEstimator(estimator=DecisionTreeRegressor(max_depth=10, min_samples_leaf=1, min_samples_split=19)),
    RidgeCV()
)
...
X_unseen = pd.read_csv('C:/Users/Lenovo/Desktop/KodSystems/TPOT/Unseen/Unseen - H2 -  Henrys - 6872.csv', sep=',')
