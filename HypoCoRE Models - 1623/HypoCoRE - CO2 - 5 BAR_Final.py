...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/HypoCoRE - HighHenry/HypoCoRE - CO2 - 5 BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('CO2-5 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CO2-5 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -1.0891107667086914
exported_pipeline = ExtraTreesRegressor(bootstrap=False, max_features=1.0, min_samples_leaf=1, min_samples_split=3, n_estimators=100)
