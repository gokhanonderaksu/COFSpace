...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/HypoCoRE - HighHenry/HypoCoRE - CO2 - 0.1 BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('CO2-0.1bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CO2-0.1bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.015384209132800471
exported_pipeline = XGBRegressor(learning_rate=0.1, max_depth=3, min_child_weight=1, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.6500000000000001, verbosity=0)
...
X_unseen = pd.read_csv('C:/Users/Lenovo/Desktop/KodSystems/TPOT/Unseen/Unseen - CO2 -  Henrys - 6309.csv', sep=',')
y_pred_unseen = exported_pipeline.predict(X_unseen)
df5 = pd.DataFrame(y_pred_unseen)
df5.to_csv('HypoCOF - CO2 - Unseen.csv', index=False)

X_unseen2 = pd.read_csv('C:/Users/Lenovo/Desktop/KodSystems/TPOT/Unseen/Unseen - CO2 -  Henrys - ALL.csv', sep=',')
