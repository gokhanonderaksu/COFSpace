...
import numpy as np
from xgboost import XGBRegressor
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/HypoCoRE/HypoCoRE - CO2 - 0.1 BAR - XGB.csv', sep=',', dtype=np.float64)
...
# Average CV score on the training set was: -0.013228926567052888
exported_pipeline = make_pipeline(
    make_union(
        FunctionTransformer(copy),
        StackingEstimator(estimator=XGBRegressor(colsample_bytree=1.0, learning_rate=0.001, max_depth=9, n_estimators=100, subsample=0.5))
    ),
    XGBRegressor(colsample_bytree=0.7, learning_rate=0.1, max_depth=5, n_estimators=100, subsample=0.5)
)
