...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/HypoCoRE - HighHenry/HypoCoRE - N2 - 0.1 BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('N2-0.1bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['N2-0.1bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -5.97177735793532e-07
exported_pipeline = make_pipeline(
    StandardScaler(),
    MaxAbsScaler(),
    StackingEstimator(estimator=RandomForestRegressor(bootstrap=False, max_features=0.1, min_samples_leaf=10, min_samples_split=2, n_estimators=100)),
    FastICA(tol=0.5),
    RidgeCV()
)
