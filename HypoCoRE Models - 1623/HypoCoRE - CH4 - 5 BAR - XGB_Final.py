...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/HypoCoRE - HighHenry/HypoCoRE - CH4 - 5 BAR - XGB.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('CH4-5 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CH4-5 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.1532946364528465
exported_pipeline = make_pipeline(
    ...
    XGBRegressor(colsample_bytree=0.7, learning_rate=0.1, max_depth=6, n_estimators=100, subsample=0.7)
)
