...
tpot_data = pd.read_csv('C:/ERC Project - COF Storage/ML Excels/HypoCoRE/HypoCoRE - H2 - 10 BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('H2-10 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['H2-10 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.0010318323687429662
exported_pipeline = make_pipeline(
    StandardScaler(),
    StackingEstimator(estimator=ExtraTreesRegressor(bootstrap=True, max_features=0.2, min_samples_leaf=10, min_samples_split=17, n_estimators=100)),
    RidgeCV()
)
...
X_unseen = pd.read_csv('C:/Users/Lenovo/Desktop/KodSystems/TPOT/Unseen/Unseen - H2 -  Henrys - ALL.csv', sep=',')
